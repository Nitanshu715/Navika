...
<style>
body{{margin:0;padding:0;background:#060a14;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#e2e8f0}}
...
.btn{{display:inline-block;padding:14px 34px;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#ffffff !important;text-decoration:none;border-radius:10px;font-size:14px;font-weight:900;letter-spacing:.06em;box-shadow:0 6px 20px rgba(124,58,237,0.4)}}
.code{{background:#060a14;border:1px solid #1a2f5066;border-radius:8px;padding:10px 14px;font-size:11px;color:#64748b;word-break:break-all;margin-top:6px;font-family:monospace}}
...
def send_verification_email(to: str, name: str, token: str) -> bool:
    ...
    body    = f"""
      ...
      <div class="bw"><a href="{url}" class="btn" style="display:inline-block;padding:14px 34px;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#ffffff !important;text-decoration:none;border-radius:10px;font-size:14px;font-weight:900;letter-spacing:.06em;color:#ffffff;">◈  <span style="color:#ffffff !important;">VERIFY MY ACCOUNT</span></a></div>
...
def send_welcome_email(to: str, name: str) -> bool:
    ...
    body    = f"""
      ...
      <div class="bw"><a href="{app_url}" class="btn" style="display:inline-block;padding:14px 34px;background:linear-gradient(135deg,#00d4ff,#0284c7);color:#060c18 !important;text-decoration:none;border-radius:10px;font-size:14px;font-weight:900;letter-spacing:.06em;">→  OPEN DASHBOARD</a></div>
    """
...
def send_password_reset_email(to: str, name: str, token: str) -> bool:
    ...
    body    = f"""
      ...
      <div class="bw">
        <a href="{url}" class="btn" style="display:inline-block;padding:14px 34px;background:linear-gradient(135deg,#ef4444,#b91c1c);color:#ffffff !important;text-decoration:none;border-radius:10px;font-size:14px;font-weight:900;letter-spacing:.06em;">
